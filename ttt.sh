...
#Reference to a function in another file which directs the output of the game to the results file
source "./send_results.sh"
...
function check_game_state() {
	...
	if [ "${my_array[4]}" != "." ]; then
		if [[ "${my_array[4]}" = "${my_array[0]}" && "${my_array[4]}" = "${my_array[8]}" ]] || [[ "${my_array[4]}" = "${my_array[2]}" && "${my_array[4]}" = "${my_array[6]}" ]]; then
            ...
            if [ "${my_array[4]}" = "X" ]; then
                ...
                echo "YOU WIN!"
				send_results "Win" "Diagonal" "$time_elapsed" "$first_pick" "$choice_count"
				return 0
            fi
		fi
	fi
	...
	#Check for draws
		...
		still_playing=0
		time_elapsed=$SECONDS
		send_results "Draw" "NA" "$time_elapsed" "$first_pick" "$choice_count"
		return 0
        fi
}
